...
ds = xr.open_dataset("sediment_1D_model_91.nc", decode_times=False)
...
# ############################################################
# FlocMod on <-> advection/diff off  (profile vs depth, per Ds)
# ############################################################
...
for i, d in enumerate(range(0, nd, 6)):
    ...
    for t in range(0, nt, nt//5):
        ...
        var_all = ds.ssc.isel(time=t, Ds=d) * mass[i] * 1e3   # dims: (Nens, z)
        var_mean = var_all.mean(dim="Nens")
        var_std = var_all.std(dim="Nens")

        axs[i].fill_betweenx(z, var_mean - var_std, var_mean + var_std,
                              color=color, alpha=0.2, linewidth=0)
        axs[i].plot(var_mean, z, color=color,
                    label="T=%d min" % (ds.time[t].values/60), linewidth=2, alpha=1)
...
# ############################################################
# Total SSC profile vs depth
# ############################################################
fig, axs = plt.subplots(nrows=1, ncols=7, figsize=(12, 5), sharex=True, sharey=True)
axs = axs.flatten()

for it, t in enumerate(range(0, nt, nt//7)):
    ...
    var_all = ds.ssc.isel(time=t) * mass[i] * 1e3          # dims: (Nens, z, Ds)
    total_all = var_all.sum(dim="Ds")                       # dims: (Nens, z)
    total_mean = total_all.mean(dim="Nens")
    total_std = total_all.std(dim="Nens")

    axs[it].fill_betweenx(z, total_mean - total_std, total_mean + total_std,
                           color=color, alpha=0.2, linewidth=0)
    axs[it].plot(total_mean, z, color=color,
                 label="T=%d min" % (ds.time[t]/60), linewidth=2, alpha=1)

    axs[it].grid(alpha=0.3)
    axs[it].set_xlabel("SSC (mg/L)")
    axs[it].set_title("t=%d hrs" % (ds.time.values[t]/3600))
    axs[it].set_ylim(0, 4)
...
# ############################################################
# Grain size distribution vs time (depth-averaged)
# ############################################################
...
for i, t in enumerate(range(0, nt, nt//5)):
    ...
    var_all = ds.ssc.isel(time=t).mean(dim='z') * mass * 1e3   # dims: (Nens, Ds)
    var_mean = var_all.mean(dim='Nens')
    var_std = var_all.std(dim='Nens')

    ax.fill_between(ds.Ds.values*1e6, var_mean - var_std, var_mean + var_std,
                     color=color, alpha=0.2, linewidth=0)
    ax.plot(ds.Ds.values*1e6, var_mean, '-', color=color, linewidth=1,
            label="t=%d min" % (ds.time.values[t]/60))
...
# ############################################################
# Grain size distribution at fixed depths
# ############################################################
for dep in range(0, 50, 15):
    ...
    for i, t in enumerate(range(0, nt, nt//5)):
        ...
        var_all = ds.ssc.isel(z=dep, time=t) * mass * 1e3   # dims: (Nens, Ds)
        var_mean = var_all.mean(dim='Nens')
        var_std = var_all.std(dim='Nens')

        axs[i].fill_between(ds.Ds.values*1e6, var_mean - var_std, var_mean + var_std,
                             color=cmo.cm.haline(z[dep]/10), alpha=0.3, linewidth=0)
        axs[i].plot(ds.Ds.values*1e6, var_mean, 'k-')
...
ssc = ds.ssc.mean(dim="z").mean(dim="Nens")
ssc = ssc * mass * 1e3
h = ax.pcolormesh(time, ds.Ds.values * 1e6, ssc.T, cmap=cmo.cm.rain, vmin=0, vmax=50)
ax.set_yscale("log")
print(ssc.T)
plt.colorbar(h, orientation="vertical", label="mass (mg/L)", shrink=0.5)
fig.savefig("1DModel_HOVPLOT.png")
...
ax.grid(alpha=0.3)
time = ds.time.values / 3600
gamma = ds.gamma.mean(dim="z").mean(dim="Nens")
negative = gamma.values<1 
gamma = np.log(abs(gamma)) #* mass * 1e3
gamma.values[negative] = -gamma.values[negative]
h = ax.pcolormesh(time, ds.Ds.values * 1e6, gamma.T, cmap=cmo.cm.balance, vmin=-30, vmax=30)
ax.set_yscale("log")
print(gamma.T)
plt.colorbar(h, orientation="vertical", label="mass (mg/L)", shrink=0.5)
fig.savefig("1DModel_HOVPLOT_GAMMA.png")
...
# # /global/scratch/users/siennaw/scripts/enkf_sediment/1DModel_sediment_NOFLOCMOD.nc
# ds = xr.open_dataset("sediment_1D_model_64.nc", decode_times=False)
...
# ax.grid(alpha=0.3)
# time = ds.time.values / 3600
# ssc = ds.gamma.mean(dim="z").mean(dim="Nens")
# ssc = ssc * mass * 1e3
# h = ax.pcolormesh(time, ds.Ds.values * 1e6, ssc.T, cmap=cmo.cm.balance, vmin=-0.0001, vmax=0.0001)
# ax.set_yscale("log")
# print(ssc.T)
# plt.colorbar(h, orientation="vertical", label="mass (mg/L)", shrink=0.5)
# fig.savefig("1DModel_HOVPLOT_GAMMA.png")