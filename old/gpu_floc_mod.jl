...
# salloc --nodes 1 --qos interactive --time 01:00:00 --constraint gpu --account=m1266
...
N = 50                  # Number of sediment size classes

D = LinRange(4, 1500, N) .* 1e-6     # Sediment grain sizes (\mu m )
D = logrange(4e-6, 1500e-6, N) #.* 1e-6     # Sediment grain sizes (\mu m )
D = collect(D)
...
function my_kernel( n::CuDeviceVector{<:Real},      # # of flocs in each size class 
    ...
    println("$k -->Thread $i in block $j is running...")
...
G = 10 
@info "This is a total of $(threads_per_block * blocks) threads for our $N x $N grid"
@cuda threads=threads_per_block blocks=blocks my_kernel(sscG, massG, N, G, 10) 
CUDA.synchronize()
println("finished kernel")
