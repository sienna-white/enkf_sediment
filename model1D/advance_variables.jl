...
# advance_sediment(C_past, diffusivity, ws, gammaXdt, discretization)
function advance_sediment3(C_past, diffusivity, ws, gammaXdt, discretization) #C_past, Kz_past, sediment_type, gamma, discretization, ws)
    N = discretization["N"]
    beta = discretization["beta"]
    dt = discretization["dt"]
    dz = discretization["dz"]
    ...
    Kz_past = diffusivity #variables["Kz"]
    aA, bA, cA, dA = initialize_abcd(N)
    wsdtdz = abs(ws*dt)/dz
    # cA = zeros(N)
    ...
    bA[1] =  1 + wsdtdz   + beta/2*(Kz_past[2] + Kz_past[1]) 
    cA[1] =  -wsdtdz  - beta/2 * (Kz_past[2] + Kz_past[1])
    dA[1] =  C_past[1] + gammaXdt[1]

    # Top-Boundary: no flux for scalars
    aA[end] =  0 -beta/2 * (Kz_past[end] + Kz_past[end-1])
    bA[end] =  1 + wsdtdz   + beta/2 * (Kz_past[end] + Kz_past[end-1])  # okay adding this here 
    dA[end] = C_past[end]  + gammaXdt[end]
...
function advance_sediment(C_past, diffusivity, ws, gammaXdt, discretization)
    N = discretization["N"]
    beta = discretization["beta"]
    dt = discretization["dt"]
    dz = discretization["dz"]
    ...
    # EDIT: TAKING OUT *DT FOR GAMMA -- IT'S ALREADY THERE

    for i in 2:(N-1)
        aA[i]  = -wsdtdz - beta/2 * (Kz_past[i-1]+ Kz_past[i]) 
        bA[i]  = 1 + wsdtdz - gammaXdt[i]  + beta/2*(Kz_past[i+1] + 2*Kz_past[i] + Kz_past[i-1]) 
        cA[i]  = -beta/2 * (Kz_past[i] + Kz_past[i+1])
        dA[i]  = C_past[i]
    end 
        
    # Bottom-Boundary: no flux for scalars
    bA[1] =  1 + wsdtdz - (gammaXdt[1]) + beta/2*(Kz_past[2] + Kz_past[1]) 
    cA[1] =  -wsdtdz -beta/2 * (Kz_past[2] + Kz_past[1])
    dA[1] =  C_past[1]

    # Top-Boundary: no flux for scalars
    aA[end] =  -beta/2 * (Kz_past[end] + Kz_past[end-1])
    bA[end] =  1 - gammaXdt[end]*dt + beta/2 * (Kz_past[end] + Kz_past[end-1]) + wsdtdz # okay adding this here 
    dA[end] = C_past[end]   
