...
def main():
    ...
    for i, t in enumerate(range(0, total, max(total // 4, 1))):
        ...
        alphas = alpha0 * np.exp(0.1 *  ds.alpha.isel(time=t).values)
        betas = beta0 * np.exp(0.1 * ds.beta.isel(time=t).values)
    ...
    # =================================================================== #
    #  PLOT 18: ensemble parameter time series (mean +/- std)
    # =================================================================== #
    fig, axs = plt.subplots(figsize=(10, 4), nrows=1, ncols=2, sharex=False, sharey=False)
    axs = axs.flatten()
    shears = ds.G.values.flatten()
    shear = np.mean(shears)
    plt.suptitle(r"Avg. shear rate = %2.1f s$^{-1}$" % shear)
